...
def main(
    ...
    try:
        ...
        in_progress_tickets = [issue for issue in issues if issue["status"]["name"] == status]

        if len(in_progress_tickets) > wip_limit:
            console.print(f"[bold red]WIP limit exceeded: {len(in_progress_tickets)} > {wip_limit}[/bold red]")
            raise typer.Exit(1)
        console.print(f"[bold green]WIP limit not exceeded: {len(in_progress_tickets)} <= {wip_limit}[/bold green]")
