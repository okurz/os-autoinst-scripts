...
def test_trigger_pflash_test(mocker: MockerFixture) -> None:
    mock_openqa_cli = mocker.patch("os_autoinst_scripts.trigger_pflash_test.openqa_cli")
    mock_openqa_cli.return_value = MagicMock()
    mock_find_image = mocker.patch(
        "os_autoinst_scripts.trigger_pflash_test.find_latest_published_tumbleweed_image"
    )
