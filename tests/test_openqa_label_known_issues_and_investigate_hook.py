...
def test_hook_failed_job_bats_review(mocker: MockerFixture) -> None:
    ...
    assert result.exit_code == 0
    mock_httpx_get.assert_called_once()
    mock_subprocess_run.assert_called_once_with(["openqa-bats-review", "https://openqa.opensuse.org/tests/123"], check=True)
...
def test_hook_failed_job_label_and_bisect(mocker: MockerFixture) -> None:
    ...
    mock_investigate_and_bisect.assert_called_once_with(
        "https://openqa.opensuse.org/tests/123/issues/some_issue"
    )
